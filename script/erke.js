/*
脚本名称：通用小程序签到模板
活动入口：请修改为目标小程序/公众号名称
环境变量：通用_data（Node环境，多账号以@隔开）
使用说明：添加重写规则并打开目标小程序即可获取URL参数
更新时间：2026-04-26
*/

const $ = new Env('鸿星尔克');  // 修改为脚本显示名称
const notify = $.isNode() ? require('./sendNotify') : '';
const API_HOST = 'https://hope.demogic.com';  // TODO: 替换为实际API域名

let Message = '';
const KEY_DATA = 'target_data';  // TODO: 替换为存储键名（如 'haixin_data'）
...
// 抓取并存储 URL 数据（需要根据目标接口调整匹配逻辑）
function captureRequestURL() {
    let savedData = $.getdata(KEY_DATA) || '';
    const currentURL = $request.url;
    
    // 提取当前 URL 中的用户ID（memberId / userId / openid 等）
    const params = extractURLParams(currentURL);
    const userId = params.userId || params.memberId || params.openid;  // TODO: 调整为用户标识字段名
...
async function executeForAccount(accountNumber, url) {
    ...
    try {
        ...
        // TODO: 根据实际返回码判断签到结果（如 code == '0'）
        if (signInResult.code === '0') {
            // TODO: 解析今日获得积分/奖励
            const earnedPoints = parseInt(signInResult.data?.points || 0);
            accountMessage.push(`🎉 签到成功: 积分 +${earnedPoints}`);
        } else {
            accountMessage.push(`❌ 签到失败: ${signInResult.message || signInResult.msg}`);
        }

        // TODO: 查询总积分/余额（如果有）
        const totalPointsResult = await getTotalPoints(url);
        accountMessage.push(`🎉 当前积分: ${totalPointsResult.data?.total || 0}`);
...
// 构建签到请求体（示例，需根据实际接口修改）
function buildSignInRequestBody(params) {
    // TODO: 根据目标接口的请求体字段修改
    return {
        userId: params.userId || params.memberId,
...
// 发起签到请求（需要根据实际接口调整 URL 路径）
function signInRequest(requestBody) {
    // TODO: 修改签到接口路径
    const signInOptions = {
        url: `${API_HOST}/api/sign/in`,
